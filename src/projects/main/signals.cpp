...
namespace ov::sig
{
	namespace internal
	{
		...
		static bool RegisterSignals(std::initializer_list<int> sigs, OV_SIG_ACTION action)
		{
			struct sigaction sa{};
		...
		static bool InitializeForSigUsr1()
		{
			return RegisterSignals(
				{SIGUSR1},
				+[](int signum, siginfo_t *si, void *ctx) {
		...
		static bool InitializeForSigRt()
		{
			...
			return RegisterSignals(
				{
					SIG_JEMALLOC_SHOW_STATS,
					SIG_JEMALLOC_TRIGGER_DUMP,
				},
				+[](int signum, siginfo_t *si, void *unused) {
	...
	bool Initialize()
	{
		...
		::memset(internal::g_ome_version, 0, sizeof(internal::g_ome_version));
		::strncpy(internal::g_ome_version, info::OmeVersion::GetInstance()->ToString().CStr(), OV_COUNTOF(internal::g_ome_version) - 1);

		SetDumpFallbackPath(::ov_log_get_path());

		return internal::InitializeForAbortSignals() &&
			   internal::InitializeForSigUsr1() &&
			   internal::InitializeForSigRt() &&
			   internal::InitializeForSigHup() &&
			   internal::InitializeForSigTerm() &&
			   internal::InitializeForSigInt();
	}

	void SetDumpFallbackPath(const char *path)
	{
		static constexpr auto BUFFER_COUNT = OV_COUNTOF(internal::g_dump_fallback_directory);

		::memset(internal::g_dump_fallback_directory, 0, sizeof(internal::g_dump_fallback_directory));
		::strncpy(internal::g_dump_fallback_directory, path, BUFFER_COUNT - 1);

		if (internal::g_dump_fallback_directory[0] != '\0')
		{
			::strncat(internal::g_dump_fallback_directory, "/", BUFFER_COUNT - 1);
		}
		else
		{
		...
		internal::g_dump_fallback_directory[BUFFER_COUNT - 1] = '\0';
	}
}  // namespace ov::sig

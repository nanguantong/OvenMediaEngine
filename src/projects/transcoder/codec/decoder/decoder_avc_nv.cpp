...
bool DecoderAVCxNV::ReinitCodecIfNeed()
{
	...
	if (_codec_context->width != 0 && _codec_context->height != 0 && (GetParser()->width != _codec_context->width || GetParser()->height != _codec_context->height))
	{
...
void DecoderAVCxNV::CodecThread()
{
	...
	while (!_kill_flag)
	{
		...
		while (buffer.GetRemainedSize() > 0)
		{
			...
			// Parse the input data and fill the packet.
			...
			// If parsed frame is not same as the previous frame, reinitialize the codec.
